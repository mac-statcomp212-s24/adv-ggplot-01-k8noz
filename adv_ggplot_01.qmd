...
```{r, temp vis}
temp_plot <- ggplot(weather_clean) +
...
```{r, precip vis}
precip_plot <- ggplot(weather_clean) +
  ...
  geom_text(data=monthly_precip, aes (x=dateInYear, y = month_Precip, label=month_Precip), vjust = -0.5, hjust= 1) +
  scale_x_continuous(breaks = month_starts$dateInYear, labels=NULL) +
  geom_vline(data = month_starts, aes(xintercept = dateInYear), linetype = "solid", color="#D3D3D3") +
  theme_classic()+
...
```{r}
combined_plot <- grid.arrange(temp_plot, precip_plot, nrow=2, heights=c(2,1),top = "SFO weather in 2011")
```
